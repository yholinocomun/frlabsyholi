...
def jacobian_position(q, delta=0.0001):
    ...
    for i in range(n):
        ...
        # usar la copia de configuracion inicial
        dq[i] = dq[i] + delta
...
def jacobian_pose(q, delta=0.0001):
    ...
    # Determinar la cantidad de articulaciones
    n = q.size
    # Crear una matriz 7xn (3 de posicion + 4 del cuaternion)
    J = np.zeros((7, n))
    # Calcular la transformacion homogenea inicial (usando q)
    T = fkine_ur5(q)
    # Pose inicial del efector final como [x y z ew ex ey ez]
    x = TF2xyzquat(T)
    ...
    for i in range(n):
        ...
        # Transformacion homogenea luego del incremento (q+delta)
        T_inc = fkine_ur5(dq)
        # Pose del efector final luego del incremento
        x_inc = TF2xyzquat(T_inc)
        # Aproximacion del Jacobiano de pose usando diferencias finitas
        J[:, i] = (x_inc - x) / delta
...
def norm_ang(angulo_rad):
    """
    Normaliza un ángulo en radianes al rango [-pi, pi].
    """
    angulo_normalizado = math.fmod(angulo_rad + math.pi, 2 * math.pi)

    if angulo_normalizado < 0:
        angulo_normalizado += 2 * math.pi

    return angulo_normalizado - math.pi
